...
class Терминал:
    ...
    КОМПАНИЯ = "Пиццерия Фреди Фазбера"
    ...
    def __str__(self):
        ...
        """
        return f"{Терминал.КОМПАНИЯ} v0.0.1"
    ...
    def показать_меню(self):
        ...
        print(f"{Терминал.КОМПАНИЯ}\n"
                ...
                f"2. {self.меню[1]}\n"
                f"3. {self.меню[2]}")
        print(f"Для выбора укажите цифру через <ENTER>.\n"
              f"Для отмены заказа введите -1\n"
              f"Для подтверждения заказа введите 0\n")
    ...
    def принять_оплату(self):
        ...
        try:
            ...
            print(f"Вы внесли {оплата} р. Сдача: {self.рассчитать_сдачу(оплата)} р.")
